...
use extremal_identity::{canonical_payload, verify_signature};
...
fn score_graph(matrix: &AdjacencyMatrix, max_k: u32) -> ScoredGraph {
    ...
    let (canonical, aut_order) = extremal_scoring::automorphism::canonical_form(matrix);

    // CID from canonical graph6
    let canonical_g6 = extremal_graph::graph6::encode(&canonical);
